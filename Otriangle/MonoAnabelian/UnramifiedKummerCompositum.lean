...
-- This packages the equality of two dependent fixed fields together with finite-inertia lifting.
/-- The compositum carries the Kummer inertia generator, and its finite tame character is the
reduction of the selected primitive root from the unramified layer. -/
theorem exists_unramifiedKummerInertiaGenerator_tameCharacter
    (E : G.ResidueFiniteGaloisIntermediateField)
    (pi : LocalIntegerRing G.presentation)
    (hpi : Ideal.span {pi} = localMaximalIdeal G.presentation)
    (n : ℕ) (hn : 0 < n)
    ...
    letI : TopologicalSpace M := G.fixedFieldTopologicalSpace W
    letI : IsNonarchimedeanLocalField M := G.fixedFieldIsNonarchimedeanLocalField W
    letI : Algebra K L := (G.fixedFieldBaseExtension V).algebra
    letI : ValuativeExtension K L := (G.fixedFieldBaseExtension V).valuativeExtension
    letI : Algebra K M := (G.fixedFieldBaseExtension W).algebra
    letI : ValuativeExtension K M := (G.fixedFieldBaseExtension W).valuativeExtension
    letI : FiniteDimensional K M := (G.fixedFieldBaseExtension W).finiteDimensional
    letI : Algebra L M := (G.fixedFieldFiniteExtension
      (G.residueUnramifiedIndex_le_unramifiedKummerIndex E pi hpi n hn)).algebra
    letI : ValuativeExtension L M := (G.fixedFieldFiniteExtension
      (G.residueUnramifiedIndex_le_unramifiedKummerIndex E pi hpi n hn)).valuativeExtension
    letI : FiniteDimensional L M := (G.fixedFieldFiniteExtension
      (G.residueUnramifiedIndex_le_unramifiedKummerIndex E pi hpi n hn)).finiteDimensional
    ...
    letI : MulSemiringAction (M ≃ₐ[K] M) 𝒪[M] :=
      SpectralLocalField.integerMulSemiringAction K K M
    ∃ tau : (IsLocalRing.maximalIdeal 𝒪[M]).inertia (M ≃ₐ[K] M),
      ∃ zeta : 𝒪[M]ˣ,
        (zeta : M) = algebraMap L M hroot.choose ∧
        FiniteTameRamification.tameCharacter 𝒪[M] (M ≃ₐ[K] M) tau =
          Units.map (IsLocalRing.residue 𝒪[M]).toMonoidHom zeta := by
  let U := G.residueUnramifiedOpenSubgroupOf E
  let V : G.OpenSubgroupIndex := OrderDual.toDual U
  let T := G.unramifiedKummerOpenSubgroup E pi hpi n hn
  let W : G.OpenSubgroupIndex := OrderDual.toDual T
  ...
  letI : IsScalarTower (LocalIntegerRing K) 𝒪[M] M :=
    IsScalarTower.of_algebraMap_eq' rfl
  letI : IsScalarTower (LocalIntegerRing LP) 𝒪[M] M :=
    IsScalarTower.of_algebraMap_eq' rfl
  letI : MulSemiringAction (M ≃ₐ[K] M) 𝒪[M] :=
    SpectralLocalField.integerMulSemiringAction K K M
  letI : Algebra M A := MP.algebra
  ...
  let toN : M →ₐ[L] N :=
    { toFun := fun x => ⟨x, by
        have hx : (x : A) ∈ M := x.property
        exact Eq.mp
          (congrArg (fun F : IntermediateField K A => ((x : A) ∈ F)) hMN.symm) hx⟩
      map_one' := rfl
      map_mul' := fun _ _ => rfl
      map_zero' := rfl
      map_add' := fun _ _ => rfl
  ...
  have htoN : Function.Bijective toN := by
    constructor
    · intro x y hxy
      exact Subtype.ext (congrArg (fun z : N => (z : A)) hxy)
    · intro y
      have hy : (y : A) ∈ N.restrictScalars K := y.property
      have hy' : (y : A) ∈ M := Eq.mp
        (congrArg (fun F : IntermediateField K A => ((y : A) ∈ F)) hMN) hy
      exact ⟨⟨y, hy'⟩, Subtype.ext rfl⟩
  let e : M ≃ₐ[L] N := AlgEquiv.ofBijective toN htoN
  let sigmaL : M ≃ₐ[L] M := (AlgEquiv.autCongr e).symm
    (LCFT.uniformizerKummerGeneratorAutomorphism LP piL
      (G.residueUnramifiedOf_map_uniformizer_span E pi hpi) n hn hroot)
  let alpha : M := e.symm (LCFT.uniformizerKummerRoot LP piL n)
  have hsigmaL_alpha : sigmaL alpha = algebraMap L M hroot.choose * alpha := by
    apply e.injective
    simp only [sigmaL, alpha, AlgEquiv.autCongr_symm, AlgEquiv.autCongr_apply,
      AlgEquiv.trans_apply, AlgEquiv.symm_apply_apply, e.apply_symm_apply, map_mul,
      e.commutes]
    rw [LCFT.uniformizerKummerGeneratorAutomorphism_apply_root]
    rfl
  let I : Subgroup (M ≃ₐ[K] M) :=
    (IsLocalRing.maximalIdeal 𝒪[M]).inertia (M ≃ₐ[K] M)
  have hmapI : G.spectralInertiaSubgroup.map
  ...
  have hfixL (tau : I) (x : L) : tau.1 (algebraMap L M x) = algebraMap L M x := by
    have htau : tau.1 ∈ G.spectralInertiaSubgroup.map
      ...
      rw [hmapI]
      exact tau.property
    obtain ⟨rho, hrho, hrho_tau⟩ := htau
    have hrhoU : rho ∈ U.toSubgroup :=
      G.spectralInertiaSubgroup_le_residueUnramifiedOpenSubgroupOf E hrho
    apply Subtype.ext
    rw [← hrho_tau]
    change algebraMap M A
        ((AlgEquiv.restrictNormalHom M rho) (algebraMap L M x)) =
      algebraMap M A (algebraMap L M x)
    have hy : algebraMap M A (algebraMap L M x) = ((x : L) : A) := by
      change @algebraMap (G.fixedField W)
          (G.fixedFieldPointed W).algebraicClosure _ _
      ...
      rfl
    calc
      algebraMap M A ((AlgEquiv.restrictNormalHom M rho) (algebraMap L M x)) =
          rho (algebraMap M A (algebraMap L M x)) :=
        AlgEquiv.restrictNormal_commutes rho M (algebraMap L M x)
      _ = rho ((x : L) : A) := by rw [hy]
      _ = ((x : L) : A) := (IntermediateField.mem_fixedField_iff
        (H := U.toSubgroup) ((x : L) : A)).mp x.property rho hrhoU
      _ = algebraMap M A (algebraMap L M x) := hy.symm
  let fromI : I → (M ≃ₐ[L] M) := fun tau =>
    { toRingEquiv := tau.1.toRingEquiv
      commutes' := hfixL tau }
  have hfromIinj : Function.Injective fromI := by
    intro rho tau h
    apply Subtype.ext
    apply AlgEquiv.ext
    intro x
  ...
  have hIcard : Nat.card I = n := by
    calc
      Nat.card I = EKM.relativeRamificationIndex :=
        G.fixedFieldFiniteInertia_card T
          (G.unramifiedKummerOpenSubgroup_normal E pi hpi n hn)
      _ = n := G.unramifiedKummer_relativeRamificationIndex_eq
        E pi hpi n hn hroot
  have hAutcard : Nat.card (M ≃ₐ[L] M) = n := by
    rw [IsGalois.card_aut_eq_finrank L M]
    exact G.unramifiedKummer_fixedField_finrank_over_unramified
      E pi hpi n hn hroot
  have hfromIbij : Function.Bijective fromI :=
    (Nat.bijective_iff_injective_and_card fromI).mpr
      ⟨hfromIinj, hIcard.trans hAutcard.symm⟩
  obtain ⟨tau, htau⟩ := hfromIbij.surjective sigmaL
  have htau_alpha : tau.1 alpha = algebraMap L M hroot.choose * alpha := by
    change fromI tau alpha = _
    rw [htau]
    exact hsigmaL_alpha
  have halpha_pow : alpha ^ n = algebraMap K M (pi : K) := by
    apply e.injective
    rw [map_pow]
    simp only [alpha, e.apply_symm_apply]
    rw [LCFT.uniformizerKummerRoot_pow]
    change algebraMap L N (algebraMap K L (pi : K)) =
      e (algebraMap K M (pi : K))
    rw [← e.commutes]
    congr 1
  have halpha_integral : IsIntegral (LocalIntegerRing K) alpha := by
    let f : (LocalIntegerRing K)[X] := X ^ n - C pi
  ...
  have halphaO_pow : alphaO ^ n =
    ...
    apply Subtype.ext
    exact halpha_pow
  have halpha_irred : Irreducible alphaO := by
    have hpows : Ideal.span {alphaO} ^ n = localMaximalIdeal MP ^ n := by
      calc
        ...
        _ = (localMaximalIdeal K).map
            (algebraMap (LocalIntegerRing K) (LocalIntegerRing MP)) := by
          rw [← hpi, Ideal.map_span, Set.image_singleton]
        _ = localMaximalIdeal MP ^ EKM.relativeRamificationIndex :=
          EKM.map_localMaximalIdeal_eq_pow
        _ = localMaximalIdeal MP ^ n := by
          rw [G.unramifiedKummer_relativeRamificationIndex_eq E pi hpi n hn hroot]
    have hspan : Ideal.span {alphaO} = localMaximalIdeal MP :=
      pow_left_injective hn.ne' hpows
    exact (IsDiscreteValuationRing.irreducible_iff_uniformizer alphaO).mpr hspan.symm
  let froot : (LocalIntegerRing LP)[X] := X ^ n - C 1
  have hzeta_integral : IsIntegral (LocalIntegerRing LP)
      (algebraMap L M hroot.choose) := by
    refine ⟨froot, monic_X_pow_sub_C _ hn.ne', ?_⟩
    change Polynomial.aeval (algebraMap L M hroot.choose) froot = 0
    rw [Polynomial.aeval_def, Polynomial.eval₂_sub, Polynomial.eval₂_X_pow,
      Polynomial.eval₂_C]
    rw [← map_pow, (mem_primitiveRoots hn).mp hroot.choose_spec |>.pow_eq_one]
    simp only [map_one, sub_self]
  let zetaO : LocalIntegerRing MP :=
    ⟨algebraMap L M hroot.choose, Valuation.Integers.mem_of_integral
      (Valuation.integer.integers (valuation M)) hzeta_integral.tower_top⟩
  have hzetaO_pow : zetaO ^ n = 1 := by
    apply Subtype.ext
    change (algebraMap L M hroot.choose) ^ n = 1
    rw [← map_pow, (mem_primitiveRoots hn).mp hroot.choose_spec |>.pow_eq_one,
      map_one]
  ...
  let zeta : 𝒪[M]ˣ := hzetaO_unit.unit
  have hzeta_val : (zeta : M) = algebraMap L M hroot.choose := by
    rw [IsUnit.unit_spec]
  have halpha_mul : alphaO * (zeta : 𝒪[M]) = tau.1 • alphaO := by
    apply Subtype.ext
    change alpha * algebraMap L M hroot.choose = tau.1 alpha
    rw [htau_alpha]
    exact mul_comm _ _
  refine ⟨tau, zeta, hzeta_val, ?_⟩
  exact FiniteTameRamification.tameCharacter_eq_residue_of_uniformizer_mul
    𝒪[M] (M ≃ₐ[K] M) tau alphaO halpha_irred zeta halpha_mul
