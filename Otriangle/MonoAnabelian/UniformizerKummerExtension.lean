...
/-- Base change does not alter the roots, in a common overfield, of `X ^ n - a`. -/
theorem rootSet_X_pow_sub_C_algebraMap
    {F E A : Type*} [Field F] [Field E] [Field A]
    [Algebra F E] [Algebra F A] [Algebra E A] [IsScalarTower F E A]
    (a : F) (n : ℕ) (hn : 0 < n) :
    (X ^ n - C (algebraMap F E a) : E[X]).rootSet A =
      (X ^ n - C a : F[X]).rootSet A := by
  ext x
  rw [(monic_X_pow_sub_C (algebraMap F E a) hn.ne').mem_rootSet,
    (monic_X_pow_sub_C a hn.ne').mem_rootSet]
  simp only [aeval_def, eval₂_sub, eval₂_X_pow, eval₂_C]
  rw [IsScalarTower.algebraMap_apply F E A]
...
/-- The embedded splitting field is the intermediate field generated by all roots in the chosen
algebraic closure. -/
theorem uniformizerKummerIntermediateField_eq_adjoin_rootSet
    (K : PointedMixedCharLocalField.{u}) (pi : LocalIntegerRing K) (n : ℕ) :
    uniformizerKummerIntermediateField K pi n =
      IntermediateField.adjoin K
        ((X ^ n - C (pi : K) : K[X]).rootSet K.algebraicClosure) := by
  let p : K[X] := X ^ n - C (pi : K)
  letI : IsAlgClosed K.algebraicClosure := IsAlgClosure.isAlgClosed K
  let i : p.SplittingField →ₐ[K] K.algebraicClosure := IsAlgClosed.lift
  change i.fieldRange = IntermediateField.adjoin K (p.rootSet K.algebraicClosure)
  apply IntermediateField.toSubalgebra_injective
  rw [IntermediateField.adjoin_toSubalgebra]
  exact (IsSplittingField.adjoin_rootSet_eq_range
    (L := p.SplittingField) p i).symm
